...
**Styling**: Chakra UI v3. The design system lives in `src/theme/` — `index.ts` holds global CSS, color tokens, semantic tokens, text styles and layer styles, `recipes/` extends the built-in single-part recipes, and `slot-recipes/` the multi-part ones. Components style themselves with `variant`, `textStyle` and `layerStyle` props rather than `css` objects. `src/styles.css` holds only the font imports and the nav link classes. Theme (light/dark/auto) is toggled via `localStorage` and resolved by an inline script injected in `__root.tsx` to prevent flash.
