export const autoSaveDebounceMs = 600

export const videoBucket = 'videos'

export const videoUploadUrlExpirySec = 60 * 60

export const videoDurationProbeTimeoutMs = 15000

export const videoDurationProbeSeekSec = 1e7

export const maxVideoDurationSec = 24 * 60 * 60

export const maxVideoDimensionPx = 16384

export const assetSweepCron = '*/15 * * * *'

export const assetSweepBatchSize = 100

export const assetDeleteBatchSize = 10

export const assetDeleteRetryLimit = 5

export const assetDeleteRetryDelaySec = 30

export const workerStopTimeoutMs = 30000
